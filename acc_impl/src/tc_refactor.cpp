...
typedef struct offset_type {int data[2];} int64;
...
void loadOffset(int length, int64* offset_list_1, int64* offset_list_2, hls::stream<int512>& edgeStrm,
...
#pragma HLS unroll
            if (i*T + 2*j <= length) {
                ...
                int a_offset = offset_list_1[a_value].data[0];
                int a_length = offset_list_1[a_value].data[1] - offset_list_1[a_value].data[0];
                length_value.data[2*j] = a_length;
                offset_value.data[2*j] = a_offset;
                int b_value = edge_value.data[j*2 + 1];
                int b_offset = offset_list_2[b_value].data[0];
                int b_length = offset_list_2[b_value].data[1] - offset_list_2[b_value].data[0];
...
void loadAdjList(int length, hls::stream<int512>& offsetStrm, hls::stream<int512>& lengthStrm, hls::stream<int512>& edgeStrmIn, \
    ...
    for (int i = 0; i < loop; i++) {
        ...
        for (int j = 0; j < T_2; j++) {
            if (edge_value.data[2*j] == list_a_tag[0]) { // list_a_tag[0]: value
                ...
                for (int ii = 0; ii < list_a_tag[1]; ii++) { // list_a_tag[1]: length
...
#pragma HLS unroll
                ...
                for (int ii = o_begin_a; ii < o_end_a; ii++) {
...
#pragma HLS unroll
        ...
        for (int j = 0; j < T_2; j++) {
            ...
            for (int ii = o_begin_b; ii < o_end_b; ii++) {
...
void processList(int length, int list_a[T_2][T][BUF_DEPTH], int list_b[T_2][T][BUF_DEPTH], \
...
#pragma HLS array_partition variable=list_a_cache type=complete dim=1 dim=2
    int list_a_tag[2];
    ...
    loadAdjList(length, offsetStrm, lengthStrm, edgeStrmOut, column_list_1, column_list_2, \
                list_a_cache, list_a_tag, offsetStrmOut, lengthStrmOut, list_a, list_b);
    processList(length, list_a, list_b, offsetStrmOut, lengthStrmOut, triCount);
